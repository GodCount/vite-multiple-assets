...
var MergedStream = class extends _stream2.PassThrough {
  ...
  add(stream) {
    ...
    (_a = __privateGet(this, _onFinished)) != null ? _a : __privateSet(this, _onFinished, onMergedStreamFinished(this, __privateGet(this, _streams)));
    endWhenStreamsDone({
...
var onMergedStreamEnd = async (passThroughStream, { signal }) => {
  await _promises.finished.call(void 0, passThroughStream, { signal, cleanup: true });
};
var onInputStreamsUnpipe = async (passThroughStream, streams, { signal }) => {
  try {
    for (var iter = __forAwait(_events.on.call(void 0, passThroughStream, "unpipe", { signal })), more, temp, error; more = !(temp = await iter.next()).done; more = false) {
...
var onInputStreamEnd = async ({ passThroughStream, stream, streams, ended, aborted, controller: { signal } }) => {
  try {
    await _promises.finished.call(void 0, stream, { signal, cleanup: true, readable: true, writable: false });
...
function isTypeSync(fsStatType, statsMethodName, filePath) {
  ...
  try {
    return _fs2.default[fsStatType](filePath)[statsMethodName]();
  } catch (error) {
    if (error.code === "ENOENT") {
      return false;
    }
...
var applyBaseToPattern = (pattern, base) => isNegativePattern(pattern) ? "!" + _path2.default.posix.join(base, pattern.slice(1)) : _path2.default.posix.join(base, pattern);
var parseIgnoreFile = (file, cwd) => {
  const base = slash(_path2.default.relative(cwd, _path2.default.dirname(file.filePath)));
  return file.content.split(/\r?\n/).filter((line) => line && !line.startsWith("#")).map((pattern) => applyBaseToPattern(pattern, base));
};
var toRelativePath = (fileOrDirectory, cwd) => {
  cwd = slash(cwd);
  if (_path2.default.isAbsolute(fileOrDirectory)) {
    if (slash(fileOrDirectory).startsWith(cwd)) {
      return _path2.default.relative(cwd, fileOrDirectory);
    }
...
var normalizeOptions = (options = {}) => {
  var _a, _b;
  return {
    cwd: (_a = toPath(options.cwd)) != null ? _a : _process2.default.cwd(),
    suppressErrors: Boolean(options.suppressErrors),
    deep: typeof options.deep === "number" ? options.deep : Number.POSITIVE_INFINITY,
    ignore: [...(_b = options.ignore) != null ? _b : [], ...defaultIgnoredDirectories]
  };
};
var isIgnoredByIgnoreFiles = async (patterns, options) => {
  const { cwd, suppressErrors, deep, ignore } = normalizeOptions(options);
  const paths = await (0, import_fast_glob.default)(patterns, __spreadValues({
    cwd,
    suppressErrors,
    deep,
    ignore
  ...
  const files = await Promise.all(
    paths.map(async (filePath) => ({
      filePath,
      content: await _promises4.default.readFile(filePath, "utf8")
    }))
  );
...
var isIgnoredByIgnoreFilesSync = (patterns, options) => {
  ...
  const files = paths.map((filePath) => ({
    filePath,
    content: _fs2.default.readFileSync(filePath, "utf8")
  }));
  return getIsIgnoredPredicate(files, cwd);
};
...
var normalizePathForDirectoryGlob = (filePath, cwd) => {
  ...
  return _path2.default.isAbsolute(path2) ? path2 : _path2.default.join(cwd, path2);
};
var getDirectoryGlob = ({ directoryPath, files, extensions }) => {
  ...
  return files ? files.map((file) => _path2.default.posix.join(directoryPath, `**/${_path2.default.extname(file) ? file : `${file}${extensionGlob}`}`)) : [_path2.default.posix.join(directoryPath, `**${extensionGlob ? `/*${extensionGlob}` : ""}`)];
};
...
var checkCwdOption = (cwd) => {
  ...
  let stat;
  try {
    stat = _fs2.default.statSync(cwd);
  } catch (e) {
    return;
  }
  if (!stat.isDirectory()) {
...
var normalizeOptions2 = (options = {}) => {
  ...
  options = __spreadProps(__spreadValues({}, options), {
    ignore: (_a = options.ignore) != null ? _a : [],
    expandDirectories: (_b = options.expandDirectories) != null ? _b : true,
...
var createFilterFunction = (isIgnored) => {
  ...
  return (fastGlobResult) => {
    var _a;
    const pathKey = _path2.default.normalize((_a = fastGlobResult.path) != null ? _a : fastGlobResult);
...
exports.__spreadValues = __spreadValues; exports.__spreadProps = __spreadProps; exports.__require = __require; exports.__commonJS = __commonJS; exports.__toESM = __toESM; exports.globby = globby;
